...
>>>>>>> fc6d1529442dcf6e96e7251a18ac264e24dabcc8
...
            />
             {/* REMOVED onChange from hidden input as it was unnecessary and potentially buggy */}
...
                onChange={(e) => setCustomerDetails({ ...customerDetails, pincode: e.target.value.replace(/\D/g, '') })} // ADDED: Restrict Pincode to digits only
...
    const [customService, setCustomService] = useState(expertDetails.customService || ''); // FIX: Use expertDetails.customService for initial state

    // FIX: Use useEffect to ensure serviceAreas is always an array when component mounts/receives props
    useEffect(() => {
        if (!expertDetails.serviceAreas || !Array.isArray(expertDetails.serviceAreas)) {
            setExpertDetails(prev => ({ ...prev, serviceAreas: [] }));
        }
    }, [expertDetails.serviceAreas, setExpertDetails]);
    

    const addServiceArea = () => {
        // FIX: Add basic validation to ensure fields are not empty before proceeding
...
            const newArea = { city: newServiceCity, pincode: newServicePincode };
            // FIX: Use optional chaining (?.) for safety before accessing length and filter
            const isDuplicate = expertDetails.serviceAreas?.some(area => area.pincode === newServicePincode);
            
            if (!isDuplicate) {
                setExpertDetails(prev => ({ // FIX: Use functional update to ensure latest state
                    ...prev,
                    serviceAreas: [...(prev.serviceAreas || []), newArea] // FIX: Ensure prev.serviceAreas is used or defaults to []
                }));
...
        setExpertDetails(prev => ({ // FIX: Use functional update
            ...prev,
            serviceAreas: (prev.serviceAreas || []).filter(area => area.pincode !== pincodeToRemove) // FIX: Ensure it's an array
        }));
    };
    
...
        
        // Clear custom service if they switch away from 'Other' in the expertDetails state
        if (selectedTrade !== 'Other') {
            setCustomService('');
            setExpertDetails(prev => ({ ...prev, customService: '' }));
        } else {
            // Restore customService if returning to 'Other' or if it was set initially
             setCustomService(expertDetails.customService || ''); 
        }
    };
...
    // REMOVED: The block below was causing infinite loops because setExpertDetails was called during render.
...
                        onChange={(e) => setNewServicePincode(e.target.value.replace(/\D/g, ''))} // FIX: Restrict Pincode to digits only
...
                {/* List of Added Service Areas */}
                {/* FIX: Use optional chaining to safely access serviceAreas array and length */}
                {expertDetails.serviceAreas?.length > 0 && (
                    <div className="space-y-2 max-h-40 overflow-y-auto">
                        <p className="text-sm font-medium text-gray-700 mt-2">Added Areas:</p>
                        {/* FIX: Ensure serviceAreas is an array before mapping */}
...
                    </div>
                )}
                {/* FIX: Use optional chaining to safely access serviceAreas length */}
                {expertDetails.serviceAreas?.length === 0 && (
...
    // FIX: Initialize serviceAreas as an empty array to avoid null/undefined errors in ExpertFields logic
...
        }
        ...
        // FIX: Use optional chaining to safely check serviceAreas length
...
        // FIX: Add basic validation for Customer fields (assuming they are required fields)
...
        // FIX: Add basic validation for Expert fields (assuming they are required fields)
        ...
        // FIX: Add basic validation for Admin fields (assuming accessKey is required)
...
            // Clean up temporary customService field before sending
            // FIX: Ensure customService exists before deleting it to prevent issues if it was never set
...
            
            // FIX: Simplified token extraction
...
                            type="tel"
                            value={phone}
                            // FIX: Restrict Phone input to digits (allows other characters too, but good practice for phone number entry)
...
                        <select
                            value={role}
                            // FIX: Reset role-specific states when role changes to avoid stale data issues
                            onChange={(e) => {
                                setRole(e.target.value);
                                setCustomerDetails({ country: 'India' });
                                setExpertDetails({ serviceAreas: [], country: 'India' });
                                setAdminDetails({});
                            }}
                            className="w-full px-4 py-3 rounded-xl border border-gray-300 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all duration-300"
                            required
